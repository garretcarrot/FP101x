...
size :: Rose a -> Int
size (_ :> []) = 1
size (root :> children) = 1 + sum (map size children)

leaves :: Rose a -> Int
leaves (_ :> []) = 1
leaves (root :> children) = sum (map leaves children)
...
class Functor f => Foldable f where
  ...
  foldMap converter foldable = 
    fold $ fmap converter foldable
  
instance Foldable Rose where
  fold (x :> []) = mappend x mempty
  fold (x :> children) = foldr mappend x (map fold children)
